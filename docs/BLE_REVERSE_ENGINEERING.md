...
| 36 | `hr_fixed_8_8` (u16 LE) — **raw**, NOT sub-bpm HR | Long read as a **higher-precision heart rate** (`value/256`, corr **0.989** with `heart_rate@22` over ~258k records). **That reading is wrong (#845).** A LE u16 at 36 is `frame[37]<<8 \| frame[36]`, so `value/256` is exactly `frame[37] + frame[36]/256`: the integer part **is** the HR-like byte at `@37` — which is where all of the correlation comes from — and the "sub-bpm fraction" is the unpinned byte at `@36` over 256. Worked example, the same one that used to support the claim: `25997` = `frame[37]=101`, `frame[36]=141`, and `101 + 141/256 = 101.55`. There is **no sub-bpm HR in v18**. The decoder key stays `hr_fixed_8_8` (pinned by `decoder_oracle.json` and the CLI on both platforms); storage banks it under the neutral `V18AuxSlot.rawU16At36`. |
