...
This retires the largest standing liability in the tree: the benchmark that justifies the shipped
sleep-staging default exists only as a numeric claim inside a source comment. No dataset loader, no
script, and no test in this repository reproduces it. A number with no runnable provenance is a
