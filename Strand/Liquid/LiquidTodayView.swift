        ...
        VStack(alignment: .leading, spacing: 0) {
            ZStack(alignment: .topTrailing) {
                ...
                HStack(spacing: NoopMetrics.space2) {
                    ...
                    LiquidAddButton()
                    // #245 asked for sync indication "near the battery percentage". The battery control
                    // now carries that state itself, so this row no longer needs a separate sync chip.
